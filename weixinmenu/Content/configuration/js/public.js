    ...
    switch(arrtype)
        {
	        ...
	        case "picEdit":
	            ...
	            break;
	        case "picEditDifferentFolder":
	            new  Ajax.Request('../../configuration/system/iframe.aspx?FileType=picEditDifferentFolder',options);
	            break;
	        case "date":
				//alert('date');
...
function setColor(color)
{
	...
	if (ColorImg && color.length>1){
	    ColorImg.src=src='/images/webimages/sysImages/FileIcon/Rect.gif';
		ColorImg.style.backgroundColor = color;
	}else if(color=='#'){ ColorImg.src='/images/webimages/sysImages/FileIcon/rectNoColor.gif';}
...
function show(type,obj,title,label_width,height)
{
    ...
    <td style=\"width:20px\">\
    <img src=\"/images/webimages/sysImages/normal/close.gif\" style=\"cursor:pointer;\" title=\"close\" onclick=\"closediv($('LabelDivid'));\" />\
    </td>\
    </tr>\
    </table>\
    </div>\
...
//判断是否日期为YYYY-MM-DD,或YYYY/MM/DD格式
function isDate(str)
{
